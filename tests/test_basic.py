...
def test_load_save():
    data1 = yaml_rs.load("testdata/garden.yaml")
    yaml_rs.save("testresults/garden.yaml", data1[0])
    data2 = yaml_rs.load("testresults/garden.yaml")
    assert data1 == data2


def test_from_string():
    data1 = yaml_rs.load("testdata/garden.yaml")
    data2 = yaml_rs.from_string(garden)
    assert data1 == data2


def test_from_bytes():
    data1 = yaml_rs.load("testdata/garden.yaml")
    data2 = yaml_rs.from_bytes(garden.encode("utf-8"))
    assert data1 == data2
